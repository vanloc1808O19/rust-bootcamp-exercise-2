...
fn exercise3() {
    ...
    while additions.len() > 0 {
        ...
        for &element_index in &additions { // Add & before element_index to get a reference
            let addition_aux = values[element_index];
            addition += addition_aux; // Use the += operator to accumulate the sum
        }
    }
...
fn exercise4(value: u32) -> String {
    ...
    str_ref.to_string() // Convert the reference to String and return
}
...
fn exercise5() {
    ...
    let res = match my_map.get(&key) {
        Some(child) => child.to_string(), // Convert &String to String
        None => {
            let value = "3.0".to_string();
            my_map.insert(key, value.clone()); // Add .clone() to avoid referencing a temporary value
            value // Return the String directly
        }
    };
